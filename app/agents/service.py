...
def register_agent(db: Session, register_request: RegisterAgentRequest) -> AgentResponse:
    existing_agent = db.query(Agent).filter(
        Agent.email == register_request.email).first()
    if existing_agent:
        raise HTTPException(
    ...
    existing_rsac = db.query(Agent).filter(
        Agent.rsac_number == register_request.rsac_number).first()
    if existing_rsac:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    try:
        ...
        logging.info(
            f"✅ Agent registered successfully: {register_request.email}")
...
def update_agent(agent_id: UUID, update_data: UpdateAgentRequest, db: Session) -> AgentResponse:
